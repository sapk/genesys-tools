...
var (
	...
	dumpFlagZip      bool
	dumpFlagNoJSON   bool
	dumpFlagOnlyJSON bool
	dumpFlagCSV      bool
	dumpFlagFromJSON string
	dumpFlagUsername string
	dumpFlagPassword string
)
...
func init() {
	dumpCmd.Flags().BoolVarP(&dumpFlagFull, "extended", "e", false, "[WIP] Get also switch, dn, person, place, ...")
	dumpCmd.Flags().BoolVarP(&dumpFlagZip, "zip", "z", false, "zip the output folder")
	dumpCmd.Flags().BoolVar(&dumpFlagCSV, "csv", false, "output some csv table for some type (ex: Application)")
	dumpCmd.Flags().BoolVar(&dumpFlagNoJSON, "no-json", false, "Disable global json dump")
	dumpCmd.Flags().BoolVar(&dumpFlagOnlyJSON, "only-json", false, "Dump only global json")
	dumpCmd.Flags().StringVarP(&dumpFlagFromJSON, "from-json", "f", "", "Read data from JSON and not a live GAX (directory containing all json)")
	dumpCmd.Flags().StringVarP(&dumpFlagUsername, "user", "u", "default", "GAX user name")
	dumpCmd.Flags().StringVarP(&dumpFlagPassword, "pass", "p", "password", "GAX user password")
...
This command can dump multiple gax at a time. One folder for each GAX is created.
	...
	Run: func(cmd *cobra.Command, args []string) {
		for _, gax := range args {
			DumpGaxHost(gax, dumpFlagFull, dumpFlagZip, dumpFlagNoJSON, dumpFlagOnlyJSON, dumpFlagCSV, dumpFlagFromJSON, dumpFlagUsername, dumpFlagPassword)
		}
	},
}

func DumpGaxHost(gax string, dumpFull, dumpZip, dumpNoJSON, dumpOnlyJSON, dumpCSV bool, dumpFromJSON, dumpUsername, dumpPassword string) {
