...
func init() {
	dumpCmd.Flags().BoolVarP(&dumpFlagFull, "extended", "e", false, "Get also switch, dn, person, place, ...")
	dumpCmd.Flags().BoolVarP(&dumpFlagNoTimestamp, "no-timestamp", "t", false, "don't generate timestamp in output")
...
func DumpGaxHost(gax string, dumpFull, dumpZip, dumpNoJSON, dumpOnlyJSON, dumpCSV bool, dumpFromJSON, dumpUsername, dumpPassword string) {
	...
	if !dumpOnlyJSON {
		sig := "\n[//]: # (generated by genesys-tools developed by Antoine GIRARD)\n"
		if !dumpFlagNoTimestamp {
